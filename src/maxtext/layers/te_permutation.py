...
def compute_ragged_all_to_all_params(
  ...
  local_tokens_per_expert = jax.lax.dynamic_slice(
      all_shards_tokens_per_expert, start_indices=(shard_id, 0), slice_sizes=(1, num_experts)
  ).squeeze(0)
  ...
  input_offsets = jnp.concatenate([jnp.array([0], dtype=send_sizes.dtype), jnp.cumsum(send_sizes)[:-1]])
  ...
  sends_to_target = jnp.sum(
      all_shards_tokens_per_expert.reshape(num_expert_shards, num_expert_shards, local_expert_size),
      axis=2,
...
def te_permute(
  ...
  permuted_outputs, _permuted_probs, row_id_map, pad_offsets, tokens_per_expert = te_token_dispatch(
